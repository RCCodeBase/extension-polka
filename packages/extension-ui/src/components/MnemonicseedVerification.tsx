                ...
                <div className='verifyMnemonic'>
                    {wordsArray.map((word, index) => (
                        positions.includes(index + 1) ? (
                            ...
                            <div key={index} className='cardword'>{index + 1}. {word}</div>
                        )
                    ))}
                </div>
                <div className='randomwords'>
                    {randomWords.map((word, index) => (
                        wordsArray.includes(word)? ( <div key={index} className='randomwnotdrag'> {word}</div>):
                       ( <div key={index} draggable className='randomw' onDragStart={(e) => handleDragStart(e, word)}> {word}</div>)
                    ))}
                </div>
